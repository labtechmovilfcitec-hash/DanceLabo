...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QGroupBox, QFormLayout, QComboBox, QFrame, QScrollArea)
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.btn_load.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "folder.svg")))
        self.btn_load.setIconSize(QSize(18, 18))
        self.btn_camera = make_btn("  Usar Cámara",    "#1a6b8a")
        self.btn_camera.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "camera.svg")))
        ...
        self.combo_camera.setToolTip(
            ...
            "Pulsa el botón de actualizar para volver a detectar cámaras."
        )
        self.combo_camera.currentIndexChanged.connect(self._on_camera_selected)
        btn_scan = QPushButton()
        btn_scan.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "refresh.svg")))
        btn_scan.setIconSize(QSize(16, 16))
        ...
        btn_refresh.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "refresh.svg")))
        ...
        self.btn_train.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "brain.svg")))
        self.btn_train.setIconSize(QSize(20, 20))
        self.btn_play_unity = make_btn("  Reproducir (LSTM)", "#004a4a")
        self.btn_play_unity.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "brain.svg")))
        self.btn_play_unity.setIconSize(QSize(20, 20))
        self.btn_play_raw   = make_btn("  Secuencia Exacta",  "#003366")
        self.btn_play_raw.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "tape.svg")))
        self.btn_play_raw.setIconSize(QSize(20, 20))
        ...
        self.btn_live_eval.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "target.svg")))
        self.btn_live_eval.setIconSize(QSize(20, 20))
    ...
    def use_camera(self):
        # Si la cámara ya está activa y abierta, la apagamos (Toggle Off)
        if self.video_capture and self.video_capture.isOpened() and self.is_camera:
            self.timer.stop()
            self.video_capture.release()
            self.video_capture = None
            self.is_camera = False
            self.btn_camera.setText("  Usar Cámara")
            self.btn_camera.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "camera.svg")))
            self.lbl_record_status.setText("Cámara detenida.")
            self.video_label.clear()
            self.video_label.setText("Carga un video o activa la cámara para comenzar")
            return

        # Si no, iniciamos la cámara (Toggle On)
        if self.video_capture:
            self.video_capture.release()
        ...
        self.btn_camera.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "stop.svg")))
        self.play_video()
